...
def calcula_usuarios(log: list[Mensaje]) -> list[str]:
    ...
    nombres = set()
    for m in log:
        nombres.add(m.usuario)
    return sorted(nombres)

    # Por comprensión
    return sorted({m.usuario for m in log})
...
def cuenta_mensajes_por_meses(log: list[Mensaje]) -> dict[str, int]:
    ...
    :rtype: dict[str, int]
    '''
    #return Counter(str(m.fecha.month) + "/" + str(m.fecha.year) for m in log)
    return Counter(m.fecha.strftime("%m/%Y") for m in log) 

    res = defaultdict(int)
    for m in log:
        clave = str(m.fecha.month) + "/" + str(m.fecha.year)
        res[clave] += 1
    return res
...
def cuenta_mensajes_por_momento_del_dia(log: list[Mensaje]) -> dict[str, int]:
    ...
    for m in log:
        if 7 <= m.hora.hour <=13:
            res["MAÑANA"] += 1
        elif  14 <= m.hora.hour <= 20:
            res["TARDE"] += 1
        else:
            res["NOCHE"] += 1
    return res

    return Counter(momento_dia(m.hora) for m in log)
...
def calcula_media_horas_entre_mensajes(log: list[Mensaje]) -> float:
    ...
    for m1, m2 in zip(log, log[1:]):
        fecha_hora1 = datetime.combine(m1.fecha, m1.hora)
        fecha_hora2 = datetime.combine(m2.fecha, m2.hora)
